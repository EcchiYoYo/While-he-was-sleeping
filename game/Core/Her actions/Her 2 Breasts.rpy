...
label rub_breast_introduction_scene:
    ...
    python:
        ...
        did_level_up, did_persistent_level_up, coin_gain, her_global_arousal_gain = rubBreastCombinedVariableGenerator()
...
label rub_both_breasts_label:
    ...
    #
    python:
        did_level_up, did_persistent_level_up, coin_gain, her_global_arousal_gain = rubBothBreastCombinedVariableGenerator()
    show screen coins_gained_popup_screen(amount_gained = [coin_gain])
    show screen player_arousal_increase_screen(amount_gained = [her_global_arousal_gain])
    $ renpy.block_rollback() # this must be here to prevent rolling back and gaining additional persistent exp
    if did_orgasm == True:
...
label pinch_left_nipple_label:
    ...
    #
    python:
        did_level_up, did_persistent_level_up, coin_gain, her_global_arousal_gain = pinchNippleCombinedVariableGenerator()
...
label pinch_both_nipple_label:
    ...
    #
    python:
        did_level_up, did_persistent_level_up, coin_gain, her_global_arousal_gain = pinchBothNippleCombinedVariableGenerator()
    show screen coins_gained_popup_screen(amount = [coin_gain])
    show screen player_arousal_increase_screen(amount_gained = [her_global_arousal_gain])
    $ renpy.block_rollback() # this must be here to prevent rolling back and gaining additional persistent exp
    if did_orgasm == True:
